...
\begin{document}
  ...
  O número de épocas terá o mesmo número de valores supervisionado, então o treino executará 1 vez para cada observação.
        ...
        \subsection{Treino da MLP - BackPropagation para 1 sinal de saída}
            ...
            Um vetor $i$ do conjunto de dados, de tamanho $n$ de variáveis de entrada\\ $X_{0}(i),X_{1}(i),...,X{n}(i)$,\\ que são os sinais de entrada, são apresentados à rede, relacionados à um valor supervisionado $Y_i$.
            
            Em resumo o treino da rede perceptron com backpropagation obtém o sinal de saída no último neurônio $s$ da rede, através da propagação dos sinais de saída dos neurônios anteriores da rede, feito pela aplicação da combinação linear dos sinais de entrada com os pesos sinápticos em uma função de ativação. É calculado o erro quadrático $e$ deste sinal de saída em $s$ com o valor $Y_i$, e a regra do Gradiente Descendente com base neste erro é utilizada para o reajuste dos pesos sinápticos em todas as conexões de todos os neurônios. O processo é denominado regra delta generalizada. $\Delta$
            ...
            O erro quadrático do treino de uma rede MLP com Backpropagation, com 1 sinal de saída, no neurônio de saída $s$, em uma k-ésima iteração de treino em uma determinada época será:\\ 
            $ e(k) =\frac{1}{2}*( (Y_i(k)-Y_u(s))^2 ) $\\ onde $Y_u(s)$ é o sinal de saída deste neurônio de saída, e $Y_i$ é o valor de saída da observação do conjunto de dados.
            ...
            É importante observar que 1 época consiste no par treino e validação. Logo após o treino, os erros $e(k)$ serão obtidos novamente através das entradas $Y_i$ do conjunto de validação, e respostas propagadas $Y_u$ da rede já treinada, para então o $m.s.e$ ser calculado.
            ...
            $ y_{u}(j) = F(\sum_{i=1}^{n} (X_i(j)*W_{i}(j) + b) )$\\
            
           \paragraph*{Fase Feedforward}
            Nesta fase, em determinada época (K), em um vetor de $n$ variáveis $Xi$ correspondentes à um total de venda $Yi$, todos os neurônios $j$ propagam o sinal de saída através do calculo de $Y_{i}(j) = F(\sum_{i=1}^{n} (X_i(j)*W_{i}(j) + b) )$.
            Os pesos sinápticos de cada neurônio $j$ conectado à uma entrada $Xi_n$ são $Wi_(j)$. A saída estimada pela rede será $Yu_i(s)$.
            
            Se a iteração $i$ da época $K$ for a primeira, todos os pesos $Wi_(j)$ são inicializados com valores aleatórios pequenos.
           ...
           \end{figure}
            ...
            O valor de $Yu_{i}(j)$. dos neurônios da primeira camada oculta, são obtidos de forma que $Xi_{n}$ são os sinais de entrada das variáveis de 1 à $n$.
            
            O valor de $Yu_{i}(j)$. dos neurônios das próximas camadas, inclusive a de saída, utiliza o sinal de saída $yi_{u}$ dos neurônios da camada anterior conectados à $j$ como um sinal de entrada $Xi$.
            
            \paragraph*{Fase Feedbackward}
            O Processo de minimização do erro quadrático no treino, é feito pelo método do Gradiente Descendente, reajustando os pesos sinápticos $W_{i}(j)$ correspondente ao neurônio $j$ da rede, da seguinte forma:\\
            $\Delta W_{i}(j) = \eta*\delta_j*X_i(j)$\\
            
            
            
            
            
            O peso sináptico $W_i$ do neurônio j é reajustado proporcionalmente ao negativo da derivada parcial do erro deste neurônio em relação ao seu peso.\\ 
            $\delta_j= -\frac{\partial e}{\partial y_{u}(j)}$\\
