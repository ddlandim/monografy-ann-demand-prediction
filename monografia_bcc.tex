      ...
      \section{Inteligencia Artificial}
        ...
        \subsection{Redes Neurais Artificiais}
          ...
          \paragraph* {Funcionamento e treino do neurônio perceptron}
            De forma procedural, o processo do treino supervisionado do perceptron, com $m$ observações $Y$ = $(Y_{i},Y_{i+1}...,Y_{m})$ com $n$ entradas $X$ relacionadas à cada $Y_m$, ocorre da seguinte maneira: 
            
            \begin{enumerate}
              \item Coleta do valor supervisionado $Y_m$, que é a observação histórica do conjunto de dados.
              \item Coleta do vetor de variáveis $(X_{m0}, X_{m1}, ..., X_{mn})$ relacionados à saída supervisionada $Y_i$.
              \item Inicialização de todos os pesos sinápticos $(W_m0, W_{m1}, ..., W{mn})$ com valores aleatoriamente pequenos.
              ...
              \item LAÇO: Percorrer $Yd$ $ de $Y_0$ até $Y_m$, executando os passos abaixo:
              \item LAÇO: Inicializa-se contador de épocas $c_e$ = 0, enquanto $CP$ for verdadeiro E $l_e$ for menor que $c_e$ repete-se os passos abaixo:
              \item Incrementar 1 em $c_e$.
              \item Cálculo do sinal $U = \sum_{i=1}^{n}X_i*W_i+b$
              \item Aplicação de valor de $U$ na função de ativação $\delta$ obtendo $Y_u = \delta(U)$
              \item Obtenção do erro $e = Y_m-Y_u$
              \item Aplicação do critério de parada $CP$ em relação ao erro ($e$) E/OU em relação ao número $n$ de iteração.
              \item SE $CP$ for APROVADO, o algoritmo continua para o próximo valor supervisionado $Y_{i+1}$ sem chamar a função de aprendizagem, SE REPROVADO, a aprendizagem é chamada recalculando $U_{i+1}$ com $b_{i+1}=b_{i}+(\alpha*(e))$, $W_{i j+1} = W_{i j}+(\alpha*e*X_i)$  $X_{i+1}$ , e repete-se os 3 passos anteriores, aplicando-se $U_{i+1}$ em $\delta$ e verificando o erro de $Y_{i+1}$. 
            \end{enumerate}
