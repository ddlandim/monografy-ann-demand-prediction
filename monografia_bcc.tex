  ...
  \chapter{Fundamentação Teórica}
      ...
      \section{Análises Estatísticas}
        ...
        \subsection{Algoritmo de Regressão Linear Multipla}
            \paragraph{Modelo de Ressão Linear Múltipla}\\
                O modelo desejado será:\\
                $Yi$ = $\beta_0 + \beta_1*X_1 + ... + \beta_j*X_j + e$ 
            Onde $e$ representa o erro experimental.
             \paragraph{Verificação de linearidade}
                $Y=f(X1,X2,...,X k,\beta0,\beta1,...,\beta k)+E y$ é linear nos parâmetros se:
                $\frac {\partial f}{\partial \beta i} = h(X1,X2,...,X k)$
                Se não for linear, regressão é interrompida
             \paragraph{Parâmetros de regressão}
                Inicialmente adota-se as suposições: 
                $e$ = 0 , não se tem erro e que o modelo obtido será ideal.
             \paragraph{Método dos quadrados Mínimos}
                Este método procura encontrar o ajuste de toda observação $Y_i$ minimizando o quadrado da diferença entre esse valor e o dado observado.
                Para a aplicação deste método, iremos ter 3 vetores e uma matriz: 
                Vetor $Yi$ = Matriz $X_{ij}$ sendo toda a 1ª coluna $j$ tendo valor 1, vetor de tamanho $j$ de coeficientes, vetor de tamanho $i$ de erros.
                
                A solução de mínimos quadrados é alcançada através da minimização da soma dos quadrados dos residuos $e$, dada por $\sum_{i=1}^{n} e_i^2$ algebricamente representado por $e^t*e_i$,ou seja a multiplicação do erro pela sua transposta.
                A solução a se buscar, é um de coeficientes $\beta$ que são atribuídos às variaveis X, que após uma combinação linear podem predizer o valor $Y_i$
                Vetor $(\beta) = (y-X*\beta)'(y-X\beta)$.
                Derivando o vetor de coeficientes em relação à $\beta$ e igualando a 0, temos:
                $\beta = (X'X)^-1 X'y$
                
                Ao final da multiplicação das matrizes, obteremos o vetor $\beta$ que contêm todos os coeficientes para o modelo de regressão.
                
                Logo, ao entrarmos com o valor de $\beta$ no modelo $\beta_0 + \beta_1*X_1 + ... + \beta_j*X_j$ obteremos a variável $Yi$ predita.
      % ----------------------------------------------------------
 ...
 \paragraph* {Funcionamento do perceptron para logica AND}.\\
  ...
  O número de épocas terá o mesmo número de valores supervisionado, então o treino executará 1 vez para cada observação.
        ...
        A rede Perceptron Múltiplas Camadas com Retro-propagação de erro (MLP - Backpropagation), faz o ajuste dos pesos se dá através de duas fases:
  	       ...
  	       \paragraph*{MLP Backpropagation em R.U}
  	       Em \cite{Lopes2008} a rede neural perceptron de múltiplas camadas é utilizada para tratar a previsão de demanda do R.U da UFV, utilizando apenas como variáveis quantitativas as 5 ultimas observações anteriores ao dia a se analisar, e como variáveis qualitativas o dia da semana variando de segunda a sexta, conforme modelo já detalhado no método de regressão múltipla.
           ...
           \end{figure}
            ...
            \cite{Flavia2014} cita que o número de neurônios na camada de entrada principalmente, e nas camadas intermediárias, é proporcional à dimensão do espaço de observação. Logo em modelos preditivos de demanda, supracitados, observa-se que no mínimo deve ser utilizado um número de neurônios igual ao número de variáveis que influenciam no dado preditivo.
        ...
        \subsection{Treino da MLP - BackPropagation}
            No caso do problema de predição de demanda onde se busca apenas um valor de saída, que é a previsão de vendas em relação às variáveis de entrada, o cálculo é reduzido à apenas 1 neurônio na camada de saída, porém o somatório pode envolver outros neurônios de saída para outros casos.
            
            Um vetor de tamanho $n$ de dados $ X_{m0},X_{m1},...,X{mn} $, que serão os sinais de entrada do neurônio serão apresentados à rede, relacionados à um valor supervisionado $Ym$.
            
            O processo de mudança no funcionamento de cada neurônio da rede, diferentemente do que foi visto nas seções anteriores, será uma alteração do calculo do erro para um erro quadrático.
            
            O processo inicial do treino da rede perceptron com backpropagation segue os princípios de treino do um perceptron simples, aplicado à rede de perceptrons.\\
            Em algumas literaturas o processo é denominado regra delta generalizada.
            A função $\delta$ utilizada MLP - backpropagation deve ser uma Função não-linear diferenciável em todos os pontos, popularmente é usada a função sigmoide.
            
            O erro quadrático de um neurônio, em uma k-ésima época de treino será:\\ 
            $ e(k) =\frac{1}{2}*( \sum(Y_d(m)-Y_u(m))^2 ) $
            
            O principal objetivo do treino é reduzir a média ( $m.s.e$ ) dos valores de erro $e(k)$ de uma k-ésima época de todos os neurônios $n$ da rede:
            $m.s.e = \frac{1}{n} \sum_{k=1}^{n}e_j(k)$
            
            O valor $m.s.e$ será o critério de parada do treinamento de toda a rede, e deve ser observado a cada época de treinamento, assim que este valor atingir um ponto ótimo, conforme figura 14, o critério de parada será verdadeiro e o treinamento deve parar, pois a partir desse ponto a rede converge à um overfitting (diverge de uma capacidade de generalização, e começa a memorizar os dados de treino, podendo somente funcionar para eles).
            
            O Processo de minimização do erro quadrático é feito pelo método do Gradiente Descendente:\\
            $\Delta W_{ij} = -\eta*\frac{\partial e}{\partial W_{ij}}$
            
            O peso sináptico $i$ do neurônio j é reajustado proporcionalmente ao negativo da derivada parcial do erro deste neurônio em relação ao seu peso.
            
            $ y_j = \sum X_i*W_{ij} + b$\\
            $\delta_j= -\frac{\partial e}{\partial y_j}$
            $\Delta W_{ij} = \eta*\delta_j*X_i$
            
            \paragraph*{Processo de aprendizado no neurônio de saída} 
            \begin{figure}[!ht]
          	\center{
           ...
           \end{figure}
            ...
            O valor $\delta_j$ é retro-propagado aos neurônios de entrada como um peso $w_i$.
        
            \paragraph{Passo1 - Inicialização da rede}
             Todos os pesos são iniciados com valores aleatoriamente pequenos < 0.1.
            
            \paragraph{Algoritmo de Treinamento}
            Enquanto $CP$ for diferente de  Ponto ótimo de parada, faça:
            \begin{enumerate}
            	    \item Aplica se um vetor de variáveis $x_i$ e seu respectivo valor de saída $y_d$ desejado;
            		\item Calcula se as saídas dos neurônios, começando da primeira canada de entrada até o neurônio de saída.
            		\item Calcula se o erro para cada neurônio da camada de saída, se erro for aceitável, volta para 1) para iniciar uma nova época, senão continue.
            		\item Atualizar os pesos de cada processador, começando pela camada de saída, até a camada de entrada;
  ...
  \chapter{Plano de atividades para o TCC II}
    ...
    \item Estruturação do conjunto de dados do R.U do ano de 2017 conforme fundamentado na seção de regressão linear múltipla. O carregamento dos dados será realizado no Matlab, que já tem bibliotecas prontas que retornam o dia da semana baseado em uma data de entrada.    Aplicação do teste de linearidade no conjunto.    Após teste bem sucedido, a predição será realizada com os dados de treino de 2018. Anotando-se o Erro Absoluto Médio (EAM), Erro Quadrado Médio (EQM) e Raiz do Erro Quadrado Médio (REQM).
    
    \item O conjunto de dados estruturado na etapa anterior, terá um conjunto de 20\% de observações retiradas de cada metade de     cada semestre. Serão formados através do conjunto original, 3 pares de conjunto treino - validação.
    
    \item Implementar a estrutura de dados perceptron, será criado 3 perceptrons, cada perceptron será associado com cada par de conjunto-treino validação gerado aleatoriamente do conjunto original, e implementar a função de treino perceptron simples. A função treino irá reajustar os pesos de cada perceptron. Com os perceptrons com pesos reajustados, todos os 3 irão receber um vetor de entradas de variáveis preditivas de 2018, e emitirão uma saída k1, k2 e k3. Será feita uma média aritmética da saída k1, k2 e k3, obtendo-se km que será comparado com a saída desejada Yi de 2018. Anotando-se o Erro Absoluto Médio (EAM), Erro Quadrado Médio (EQM) e Raiz do Erro Quadrado Médio (REQM) de Yi com Km.
